...
# Base API URL
# API = "http://127.0.0.1:8000/api"
# API = "http://0.0.0.0:11000/api"
API = "https://tenserflow.onrender.com/api"
...
with st.sidebar:
    ...
    if st.button("➕ New Chat"):
        try:
            r = requests.post(f"{API}/chats/new")
            res = r.json()
            #st.write("DEBUG: New Chat Response", res)  # Debugging

            st.session_state["active_chat"] = res.get("id", None)
            st.session_state["messages"] = []
        except Exception as e:
            st.error(f"Unable to create a new chat: {e}")
    ...
    try:
        chats = requests.get(f"{API}/chats").json()
        #st.write("DEBUG: Chats API response", chats)  # Debugging
    except Exception as e:
        chats = []
        st.error(f"Failed to fetch chat history: {e}")

    # Initialize active_chat safely
    if "active_chat" not in st.session_state:
        if isinstance(chats, list) and len(chats) > 0 and "id" in chats[0]:
            st.session_state["active_chat"] = chats[0]["id"]
        else:
            st.session_state["active_chat"] = None
    ...
    if isinstance(chats, list):
        for c in chats:
            ...
            col1, col2 = st.columns([0.8, 0.2])
            with col1:
                if st.button(f"{title}", key=f"chat-{chat_id}", use_container_width=True):
                    st.session_state["active_chat"] = chat_id
                    st.session_state["messages"] = []
            ...
            if st.session_state.get("menu_open") == chat_id:
                new_name = st.text_input("Rename chat", value=title, key=f"rename-{chat_id}")
                col_a, col_b = st.columns(2)
...
if "messages" not in st.session_state or not st.session_state["messages"]:
    try:
        thread = requests.get(f"{API}/chats/{active}").json()
        #st.write("DEBUG: Thread Response", thread)  # Debugging
...
for m in st.session_state["messages"]:
    role_class = "user-bubble" if m.get("role") == "user" else "bot-bubble"
    st.markdown(f"<div class='{role_class}'>{m.get('content', '')}</div>", unsafe_allow_html=True)
...
if user_input:
    try:
        data = {"chat_id": active, "content": user_input}
        r = requests.post(f"{API}/chat", data=data).json()
        st.write("DEBUG: Send Message Response", r)  # Debugging
        ...
        if isinstance(r, dict):
            st.session_state["messages"].append({
                ...
                "content": r.get("content", "No response received."),
                "id": r.get("id", -2),
                "created_at": r.get("created_at", str(datetime.utcnow()))
            })
        else:
            st.session_state["messages"].append({
                "role": "assistant",
